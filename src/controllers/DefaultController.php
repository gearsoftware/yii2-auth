...
namespace gearsoftware\auth\controllers;

use gearsoftware\assets\core\CoreAsset;
use gearsoftware\auth\AuthModule;
use gearsoftware\auth\helpers\AvatarHelper;
use gearsoftware\auth\models\Auth;
use gearsoftware\auth\models\forms\ConfirmEmailForm;
use gearsoftware\auth\models\forms\LoginForm;
use gearsoftware\auth\models\forms\ResetPasswordForm;
use gearsoftware\auth\models\forms\SetEmailForm;
use gearsoftware\auth\models\forms\SetPasswordForm;
use gearsoftware\auth\models\forms\SetUsernameForm;
use gearsoftware\auth\models\forms\SignupForm;
use gearsoftware\auth\models\forms\UpdatePasswordForm;
use gearsoftware\components\AuthEvent;
...
class DefaultController extends BaseController
{
    ...
    {
        return [
            'captcha' => Yii::$app->core->captchaAction,
            'oauth' => [
                'class' => 'gearsoftware\auth\AuthAction',
            ...
            if ($auth) { // login
                ...
                if (isset($attributes['email']) && $attributes['email'] && User::find()->where(['email' => $attributes['email']])->exists()) {
                    Yii::$app->getSession()->setFlash('error', [
                        Yii::t('core/auth', "User with the same email as in {client} account already exists but isn't linked to it. Login using email first to link it.", ['client' => $client->getTitle()]),
                    ]);
    ...
    protected function createUser($attributes)
    {
        ...
        if ($user->save()) {
            ...
            if ($auth->save()) {
                ...
                Yii::$app->session->setFlash('danger', 'Error 901: ' . Yii::t('core/auth', "Authentication error occurred."));
            ...
            foreach ($fields as $field) {
                if (isset($errors[$field])) {
                    Yii::$app->session->setFlash('danger', $user->getFirstError($field));
                    return Yii::$app->response->redirect(Url::to(['/auth/default/set-' . $field]));
                }
            }

            Yii::$app->session->setFlash('danger', 'Error 902: ' . Yii::t('core/auth', "Authentication error occurred."));
            return Yii::$app->response->redirect(Url::to(['/auth/default/login']));
        }
    ...
    public function actionConfirmRegistrationEmail($token)
    {
        if (Yii::$app->core->emailConfirmationRequired) {
            ...
            throw new NotFoundHttpException(Yii::t('core/auth', 'Token not found. It may be expired'));
        }
    }
        ...
        if ($model->load(Yii::$app->request->post()) AND $model->validate()) {
            if ($this->triggerModuleEvent(AuthEvent::BEFORE_PASSWORD_RECOVERY_REQUEST, ['model' => $model])) {
                if ($model->sendEmail(false)) {
                    ...
                    Yii::$app->session->setFlash('warning', Yii::t('core/auth', "Unable to send message for email provided"));
                }
            }
        }
    ...
    public function actionResetPasswordRequest($token)
    {
        ...
        if (!$user) {
            throw new NotFoundHttpException(Yii::t('core/auth', 'Token not found. It may be expired. Try reset password once more'));
        }
    ...
    protected function triggerModuleEvent($eventName, $data = [])
    {
        ...
        Yii::$app->core->trigger($eventName, $event);
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->validate()) {
                try {
                    ...
                    return Yii::t('core', 'An unknown error occurred.');
                }
            } else {
    ...
    public function actionRemoveAvatar()
    {
        ...
        try {
            ...
            return CoreAsset::getDefaultAvatar('large');
        } catch (Exception $exc) {
            Yii::$app->response->statusCode = 400;
